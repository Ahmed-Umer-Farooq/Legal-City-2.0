...
const login = async (req, res) => {
  try {
    ...
    // Include admin status and verified status in response
    const userResponse = {
      id: user.id,
      name: user.name,
      email: user.email,
    ...
    console.log('Login response - Role:', role, 'Registration ID:', user.registration_id, 'Redirect:', redirect);
  ...
  try {
    ...
    if (user) {
      ...
      return res.json({ 
        ...
        user: { ...updated, role: updated.role || 'user' },
        redirect: '/'
      });
    }
    user = await db('lawyers').where({ id: req.user.id }).first();
    if (user) {
      await db('lawyers').where({ id: req.user.id }).update({ is_verified: 1 });
      const updated = await db('lawyers').where({ id: req.user.id }).first();
      return res.json({ 
        message: 'Status updated to Verified', 
        user: { ...updated, role: 'lawyer' },
        redirect: '/lawyer-dashboard'
      });
    }
    return res.status(404).json({ message: 'User not found' });
