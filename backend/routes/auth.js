...
router.get('/google/callback',
  ...
  (req, res) => {
    ...
    // For lawyers: redirect to dashboard if verified or completed, otherwise to setup
    if (role === 'lawyer') {
      if (completed || isVerified) {
        redirectUrl = `${process.env.FRONTEND_URL}/lawyer-dashboard?token=${token}`;
      } else {
        redirectUrl = `${process.env.FRONTEND_URL}/google-lawyer-setup?token=${token}`;
      }
    } else {
      // For users: redirect to home if completed, otherwise to setup
      if (completed || isVerified) {
        redirectUrl = `${process.env.FRONTEND_URL}/?token=${token}`;
      } else {
